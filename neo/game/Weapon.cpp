...
//	ammoAvail = owner->inventory.HasAmmo( ammoType, ammoRequired );		// NOTE: BFG
...
void idWeapon::Event_LaunchProjectiles( int num_projectiles, float spread, float fuseOffset, float launchPower, float dmgPower ) {
	...
	if ( !gameLocal.isClient ) {
		...
		/*  NOTE: THIS WAS FROM VANILLA , WAS USING #ELSE
...
//		if ( clipSize && ammoRequired && !g_infiniteAmmo.GetBool() ) {		// NOTE : BFG CODE BIT	
		if ( clipSize && ammoRequired ) {
...
//				ent->Damage( owner, owner, globalKickDir, meleeDefName, owner->PowerUpModifier( MELEE_DAMAGE ), tr.c.id );		// NOTE: THIS WAS WITH #ELSE FROM VANILLA
				hit = true;
			}
